...
static ssize_t oppo_proc_mp_lcm_on_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if ((dev_mkdir(CSV_LCM_ON_PATH, S_IRUGO | S_IWUSR)) != 0){
		ILI_ERR("Failed to create directory %s for mp_test\n", CSV_LCM_ON_PATH);
	}
	...
	if(ilits->oppo_result == 0){
		ILI_INFO("MP TEST PASS or warning\n");
	}else{
		ILI_ERR("MP TEST FAIL\n");
	}
	len = snprintf(ptr, 256,"%d error(s) %s\n", ilits->oppo_result, ilits->oppo_result ? "test failed" : "All test passed");	
...
static ssize_t oppo_proc_black_screen_test_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	int len = 0;
	int ret = 0;
	bool esd_en = ilits->wq_esd_ctrl, bat_en = ilits->wq_bat_ctrl;
...
static ssize_t oppo_proc_black_screen_test_write(struct file *filp, const char *buffer, size_t size, loff_t *pos)
{
  	int ret = 0;
	char cmd[10] = { 0 };
	int value = 0;
	...
	ret = sscanf(cmd, "%d", &value);
	if (ret < 0) {
		ILI_ERR("%s:sscanf failed\n", __func__);
	}
...
static ssize_t oppo_proc_gesture_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	uint32_t len = 0;
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "gesture : %s\n", ilits->switch_gesture_flag ? "Enable" : "Disable");
...
static ssize_t oppo_proc_gesture_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
	int ret = 0;
	int gesture_oppo = 0;
	char cmd[256] = { 0 };
	...
	gesture_oppo = ili_katoi(cmd);
	ILI_INFO("gesture_oppo = %d\n", gesture_oppo);
	if(gesture_oppo == 1){
		if(ilits->tp_suspend){
...
static ssize_t oppo_proc_game_switch_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	uint32_t len = 0;
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "%x\n", ilits->game_switch_mode);
...
static ssize_t oppo_proc_game_switch_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
	int ret = 0;
	char cmd[256] = { 0 };
	int game_cmd = 0;
...
static u32 oppo_rw_reg[5] = {0};
static ssize_t oppo_proc_register_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	int ret = 0, len = 0;
	...
	stop_mcu = oppo_rw_reg[0];
	type = oppo_rw_reg[1];
	addr = oppo_rw_reg[2];
	write_data = oppo_rw_reg[3];
	write_len = oppo_rw_reg[4];
	...
	if (ret < 0) {
		...
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Failed to enter ICE mode\n");
	}

	if (type == read) {
		if (ili_ice_mode_read(addr, &read_data, sizeof(u32)) < 0) {
			ILI_ERR("Read data error\n");
			len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Read data error\n");
		}
		...
		if (ili_ice_mode_write(addr, write_data, write_len) < 0) {
			...
			len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Write data error\n");
		}
...
static ssize_t oppo_proc_register_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
		...
		while ((token = strsep(&cur, ",")) != NULL) {
			oppo_rw_reg[count] = ili_str2hex(token);
			ILI_INFO("oppo_rw_reg[%d] = 0x%x\n", count, oppo_rw_reg[count]);
			count++;
		}
...
static ssize_t ilitek_direction_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "oppo_direction info : %d\n", ilits->direction_oppo);
...
static ssize_t ilitek_direction_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
	...
	ret = sscanf(cmd, "%d", &direction_oppo);
	if (ret < 0) {
		ILI_ERR("%s:sscanf failed\n", __func__);
	}

	//direction_oppo = cmd[0] - '0';
	ILI_INFO("direction_oppo = %d\n", direction_oppo);
	if(direction_oppo == 0){
		direction_cmd = 0x01;
		ret = ili_ic_func_ctrl("edge_palm", direction_cmd);
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
		ilits->direction_oppo = 0;
	}else if(direction_oppo == 1){
		direction_cmd = 0x02;
		ret = ili_ic_func_ctrl("edge_palm", direction_cmd);	
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
		ilits->direction_oppo = 1;
	}else if(direction_oppo == 2){
		direction_cmd = 0x00;
		ret = ili_ic_func_ctrl("edge_palm", direction_cmd);	
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
		ilits->direction_oppo = 2;
	}else{
		ILI_ERR("UNKNOW CMD\n");
	}
	mutex_unlock(&ilits->touch_mutex);
...
static int32_t c_oppo_ili_coordinate_show(struct seq_file *m, void *v)
{
	...
	char tmp[256] = {0};
	ILI_INFO("c_oppo_coordinate_show\n");
	snprintf(tmp, sizeof(tmp)-1, "%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d",
...
const struct seq_operations ili9882n_oppo_ili_coordinate_seq_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
{
	return seq_open(file, &ili9882n_oppo_ili_coordinate_seq_ops);
}

/*
static ssize_t oppo_proc_i2c_device_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	uint32_t len = 0;
	...
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "ILITEK SPI devices\n");
#else
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "NO devices\n");
#endif
...
static ssize_t ilitek_limit_control_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "tp_limit_enable : %d\n", ilits->tp_limit_enable);
...
static ssize_t ilitek_limit_control_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
	...
	ret = sscanf(cmd, "%d", &tp_limit_enable);
	if (ret < 0) {
		ILI_ERR("%s:sscanf failed\n", __func__);
	}
	...
	//direction_oppo = cmd[0] - '0';
	ILI_INFO("tp_limit = %d\n", tp_limit_enable);
	if(tp_limit_enable == 0){
		direction_cmd = 0x01;
		ret = ili_ic_func_ctrl("edge_palm", direction_cmd);
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
		ilits->tp_limit_enable = 0;
	}else if(tp_limit_enable == 1){
		direction_cmd = 0x02;
		ret = ili_ic_func_ctrl("edge_palm", direction_cmd);	
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
		ilits->tp_limit_enable = 1;
	}else if(tp_limit_enable == 2){
		direction_cmd = 0x00;
		ret = ili_ic_func_ctrl("edge_palm", direction_cmd);	
		if(ret < 0)
			ILI_ERR("write edge_palm err\n");
		ilits->tp_limit_enable = 2;
	}else{
		ILI_ERR("UNKNOW CMD\n");
	}
	mutex_unlock(&ilits->touch_mutex);
...
static ssize_t oppo_proc_incell_panel_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	uint32_t len = 0;
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "1\n");
...
static ssize_t ilitek_proc_oppo_debug_level_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
	int res = 0;
	char cmd[10] = { 0 };
	...
	ilits->oppo_debug_level = ili_katoi(cmd);

	ILI_INFO("oppo_debug_level = %d\n", ilits->oppo_debug_level);

	return size;
}

static ssize_t oppo_proc_irq_depth_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	uint32_t len = 0;
	...
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "now depth=%d\n", desc->depth);
...
static ssize_t ilitek_proc_oppo_upgrade_fw_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
{
	int ret = 0;
	int value = 0;
	char cmd[10] = { 0 };
	...
	if (ili_fw_upgrade_handler(NULL) < 0)
		ILI_ERR("FW upgrade failed during oppo fw upgrade\n");
...
static ssize_t ilitek_proc_main_register_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "%d.%d.%d.%d\n", szBuf[0], szBuf[1], szBuf[2], szBuf[3]);
...
//static ssize_t ilitek_proc_oppo_sign_firmware_write(struct file *filp, const char *buffer, size_t size, loff_t *pPos)
//{
//
//}
...
struct file_operations ili9882n_proc_oppo_register_info_fops = {
	.write =oppo_proc_register_write,
	.read = oppo_proc_register_read,
};

struct file_operations proc_oppo_tp_direction_fops = {
	.owner = THIS_MODULE,
...
struct file_operations proc_oppo_tp_limit_enable_fops =
{
...
struct file_operations proc_debug_level_oppo_fops = {
...
struct file_operations proc_sign_firmware_fops = {
	.write = ilitek_proc_oppo_sign_firmware_write,
};*/

proc_node_t proc_table_oppo[] = {
	...
	{"oppo_register_info", NULL, &ili9882n_proc_oppo_register_info_fops, false},
	{"oppo_tp_direction", NULL, &proc_oppo_tp_direction_fops, false},
	{"coordinate", NULL, &proc_coordinate_fops, false},
//	{"i2c_device_test", NULL, &proc_i2c_device_test_fops, false},
	{"oppo_tp_limit_enable", NULL, &proc_oppo_tp_limit_enable_fops, false},
	{"incell_panel", NULL, &proc_incell_panel_fops, false},
//	{"sign_firmware", NULL, &proc_sign_firmware_fops, false},
	{"debug_level", NULL, &proc_debug_level_oppo_fops, false},
...
void ili9882n_tddi_node_init(void)
{
	...
	size = ARRAY_SIZE(proc_table_oppo);
	for (; i < size; i++) {
		proc_table_oppo[i].node = proc_create(proc_table_oppo[i].name, 0666, proc_dir_oppo, proc_table_oppo[i].fops);

		if (proc_table_oppo[i].node == NULL) {
			proc_table_oppo[i].isCreated = false;
			ILI_ERR("Failed to create oppo %s under /proc\n", proc_table_oppo[i].name);
			ret = -ENODEV;
		} else {
			proc_table_oppo[i].isCreated = true;
			ILI_INFO("Succeed to create oppo %s under /proc\n", proc_table_oppo[i].name);
		}
	}
