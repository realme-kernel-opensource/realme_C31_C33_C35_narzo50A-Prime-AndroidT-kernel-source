...
#define RESUME_BY_DDI			ENABLE  //DISABLE
...
#define DMESG_SEQ_FILE			DISABLE //ENABLE
...
#define CSV_LCM_ON_PATH			"/sdcard/TpTestReport/ilitek_mp_lcm_on_log"
#define CSV_LCM_OFF_PATH		"/sdcard/TpTestReport/ilitek_mp_lcm_off_log"
...
({									\
	printk("ILITEK[INFO]: (%s, %d): " fmt, __func__, __LINE__, ##arg);	\
})									\
...
do {									\
	if (debug_en & ilits->oppo_debug_level)						\
...
//oppo mp test
	...
	bool black_flag;
	int oppo_result;
	bool switch_gesture_flag;
	int direction_oppo;
	int tp_limit_enable;
	struct work_struct rebuild_cmd;
	bool game_switch_mode;
	int oppo_debug_level;
	...
	//add lcd notify suspend_resume function
	unsigned long sleep_mode;
	struct notifier_block notifier_suspend_resume;
...
struct gesture_coordinate {
	uint8_t gesture_type;/*ritchie add for oppo*/
	u16 code;
	u8 clockwise;
	int type;
...
struct ilitek_ic_info {
	u8 type;
	u8 ver;
	u8 fw_ver_info; /*fw version for hardware info*/
	u16 id;
	u32 pid;
	u32 pid_addr;
	u32 product_id;
...
extern int ili_oppo_proc_init(void);
...
extern void ili9882n_tddi_node_init(void);
