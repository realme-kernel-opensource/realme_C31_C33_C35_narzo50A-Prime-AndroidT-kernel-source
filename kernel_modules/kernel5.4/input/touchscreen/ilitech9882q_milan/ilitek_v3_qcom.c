...
static void ipd_usb_work_queue(struct work_struct *work)
 {
	ILI_INFO("ipd_usb_work_queue +++++usb_state = %ld\n",ilits->usb_state);
	if (ilits->tp_suspend){
		ILI_INFO("suspend = %d,isUpgrading = %d,don't need usb function",ilits->tp_suspend,ilits->fw_upgrade_mode);
		return;
	}

	mutex_lock(&ilits->touch_mutex);
	if(ilits->usb_state == 1) {
		ili_ic_func_ctrl("plug", 0x0);
	} else if(ilits->usb_state == 0) {
		ili_ic_func_ctrl("plug", 0x1);
	} else
		ILI_INFO("invild usb statues value");
	mutex_unlock(&ilits->touch_mutex);
 }

static int ipd_notifie_usb(struct notifier_block *nb,unsigned long value, void *data)
{
	ilits->usb_state = value;
	ILI_INFO("value == %ld,ipd->usb_state = %ld data = %p",value,ilits->usb_state,data);
	schedule_work(&ilits->usb_work_queue);
	return 0;
}
...
{
	//int res = 0;
	ILI_INFO("ipd_usb_notifier_init\n");
	ilits->notifier_usb.notifier_call = ipd_notifie_usb;
	//res = tp_usb_register_client(&ilits->notifier_usb);
}
...
#endif
#endif
	INIT_WORK(&ilits->usb_work_queue, ipd_usb_work_queue);
...
static int __init ilitek_plat_dev_init(void)
{
	...
	printk("ILITEK TP driver init for QCOM Enter, line = %d\n", __LINE__);
	ILI_INFO("ILITEK TP driver init for QCOM\n");
	lcd_name = strstr(cmd_line, "lcd_name=");
	
	if  (lcd_name == NULL){
		printk("lcd name  is null return \n");
		return 0 ;
	}
	printk("lcd_name is %s.\n", lcd_name);
	if (strncmp("lcd_name=lcd_ili9882q_milan_hdplus_txd", lcd_name, 28) == 0) {
