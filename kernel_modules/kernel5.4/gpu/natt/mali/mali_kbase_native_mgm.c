...
{
	struct page *p;
	struct timespec64 tstart = {0,0};
	struct timespec64 tend = {0,0};
	u64 tms1 = 0;
	u64 tms2 = 0;
	/*
	...
	tms1 = 1000 * tend.tv_sec + tend.tv_nsec / 1000000;
	tms2 = 1000 * tstart.tv_sec +tstart.tv_nsec / 1000000;
	if (tms1 - tms2  > 100)
	{
		printk(KERN_INFO "sprddebug: %s(%d), timegap: %ld ms, pid(command): %d(%s), page_order: %d.\n", __func__, __LINE__,
						(tms1 - tms2), current->pid, current->comm, order);
	}
	if (!p)
	{
		printk(KERN_INFO "sprddebug: %s(%d), alloc page failed pid(command) = %d(%s)\n", __func__, __LINE__,
