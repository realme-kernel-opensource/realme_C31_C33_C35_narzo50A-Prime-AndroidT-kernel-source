		...
		enum kbase_memory_type type, int group_id)
{
	...
	if (tms1 - tms2  > 100)
	{
		printk(KERN_INFO "sprddebug: %s(%d), timegap: %ld ms, pid(command): %d(%s).\n", __func__, __LINE__,
						(tms1 - tms2), current->pid, current->comm);
	}
	if (!alloc)
	{
		printk(KERN_INFO "sprddebug: %s(%d), vzalloc failed pid(command) = %d(%s)\n", __func__, __LINE__,
