				...
				ktime_t *end_timestamp)
{
	...
	if (completion_code == BASE_JD_EVENT_STOPPED) {
		...
		if (!kbase_ctx_flag(katom->kctx, KCTX_DYING)) {
			const char *fault_keyword = kbase_gpu_exception_name(completion_code);
			dev_warn(kbdev->dev, "error detected from slot %d, job status 0x%08x (%s)",
					js, completion_code, fault_keyword);
#ifdef SPRD_SUPPORT_FAULT_KEYWORD
			now = seconds_right_now();
			if(strcmp("DONE",fault_keyword) == 0)
				time[DONE] = now;
			if(strcmp("TRANSLATION_FAULT",fault_keyword) == 0)
				time[TRANSLATION_FAULT] = now;
			if(strcmp("TERMINATED",fault_keyword) == 0)
				time[TERMINATED] = now;
			if(strcmp("JOB_READ_FAULT",fault_keyword) == 0)
				time[JOB_READ_FAULT] = now;
			if(strcmp("DATA_INVALID_FAULT",fault_keyword) == 0)
				time[DATA_INVALID_FAULT] = now;
			if(strcmp("INSTR_INVALID_ENC",fault_keyword) == 0)
				time[INSTR_INVALID_ENC] = now;
#endif
		}
