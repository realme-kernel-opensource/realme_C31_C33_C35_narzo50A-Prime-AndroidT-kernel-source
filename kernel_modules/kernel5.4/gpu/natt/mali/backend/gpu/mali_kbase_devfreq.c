...
kbase_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
	...
	kbase_pm_get_dvfs_metrics(kbdev, &kbdev->last_frequtil_metrics, &diff);
	busy_time = diff.time_busy;
	total_time = diff.time_busy + diff.time_idle;
#else
	// Fixed compiling error for  unused parameter
	kbdev = kbdev;
	busy_time=0;
	total_time=100;
#endif
	if (kbdev->enable_freq_stats) {
		trace_clock_set_rate("unisoc-gpu-freq", nominal_freq, raw_smp_processor_id());
		trace_clock_set_rate("unisoc-gpu-utilization", busy_time*100/total_time, raw_smp_processor_id());
	}
...
#endif
	if(kbdev->gpu_tz) {
		ret = kbase_platform_set_DVFS_table(kbdev);
		if (ret)
			printk(KERN_ERR "%s:kbase_platform_set_DVFS_table fail err: %d.\n", __func__, ret);
	}
	return 0;
}
...
#endif
#endif
	kbdev->gpu_tz = thermal_zone_get_zone_by_name("gpu-thmzone");
	if (!kbdev->gpu_tz) {
		dev_err(kbdev->dev,
			"Failed to get gpu thermal zone\n");
	}
