...
kbase_devfreq_status(struct device *dev, struct devfreq_dev_status *stat)
{
	...
	if (kbdev->devfreq_cooling)
		memcpy(&kbdev->devfreq_cooling->devfreq->last_status, stat,
				sizeof(*stat));
#endif
#endif
#endif
		...
		struct devfreq_dev_profile *dp)
{
	...
	ret = dev_pm_opp_of_add_table(kbdev->dev);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0))
	ret = of_init_opp_table(kbdev->dev);
#endif /* LINUX_VERSION_CODE */
	if (ret) {
		dev_err(kbdev->dev, "failed to init OPP table: %d\n", ret);
		return ret;
	}
...
int kbase_devfreq_init(struct kbase_device *kbdev)
{
	...
	*/
	kbdev->current_freq = kbase_platform_get_init_freq();
	...
	kbase_platform_limit_min_freq(kbdev->dev);
#endif
