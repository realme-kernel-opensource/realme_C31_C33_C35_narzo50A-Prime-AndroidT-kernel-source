...
static int sprd_sensor_file_open(struct inode *node, struct file *file)
{
	...
	void __iomem  *addr_scl0;
	addr_scl0 = ioremap(0x647106b8, 128);
	if (addr_scl0) {
		pr_err("before write I2C0_PIN-pull up: scl0: %lx, sda0: %lx ", readl_relaxed(addr_scl0), readl_relaxed(addr_scl0+0x4));
		writel_relaxed(0x83088, addr_scl0);//拉高
		writel_relaxed(0x83088, addr_scl0 + 0x4);//拉高
		pr_err("after write I2C0_PIN-pull up: scl0: %lx, sda0: %lx ", readl_relaxed(addr_scl0), readl_relaxed(addr_scl0+0x4));
		iounmap(addr_scl0);
	}
...
/*		wake_unlock(&p_mod->wakelock);*/
	...
	addr_scl0 = ioremap(0x647106b8, 128);
	if (addr_scl0) {
		pr_err("before write I2C0_PIN-pull dowm: scl0: %lx, sda0: %lx ", readl_relaxed(addr_scl0), readl_relaxed(addr_scl0+0x4));
		writel_relaxed(0x82044, addr_scl0);//拉低
		writel_relaxed(0x82044, addr_scl0 + 0x4);//拉低
		pr_err("after write I2C0_PIN-pull dowm: scl0: %lx, sda0: %lx ", readl_relaxed(addr_scl0), readl_relaxed(addr_scl0+0x4));
		iounmap(addr_scl0);
	}
	pr_err("after write,pull down scl0/sda0 succeed,sensorid: %d", p_dev->sensor_id);
