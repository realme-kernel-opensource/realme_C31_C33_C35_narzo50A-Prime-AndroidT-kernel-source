...
int select_sensor_mclk(unsigned int clk_set, char **clk_src_name, uint8_t *clk_div)
{
	unsigned char i = 0;
	unsigned char j = 0;
	...
	if (clk_set > 9600 || !clk_src_name || !clk_div)
		return -EPERM;
			 ...
			 int sensor_id)
{
	...
	if (set_mclk == 0  && p_dev->mclk_count) {
		...
		if (set_mclk > SPRD_SENSOR_MAX_MCLK * 100)
			set_mclk = SPRD_SENSOR_MAX_MCLK * 100;
		...
		if (p_dev->sensor_eb && p_dev->sensor_clk) {
			...
			pr_info("set_mclk %d sensor_id %d\n",
				set_mclk, sensor_id);
#if 1//def MCLK_NEW_PROCESS
			...
			ret = clk_set_rate(p_dev->sensor_clk,
				     (set_mclk * SPRD_SENSOR_MCLK_VALUE/100));
			if (ret) {
				pr_err("set rate failed\n");
				goto exit;
			}
