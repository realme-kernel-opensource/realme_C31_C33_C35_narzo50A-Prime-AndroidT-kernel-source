...
#if 0
/* hongzhenglong@ODM.HQ.Charger 2021/02/07 add hardinfo for charger_ic */
...
#if 0
char *hq_cmd_line;
int hq_get_lcd_name_from_cmdline(void)
{
	struct device_node *cmdline_node;
	int rc;

	cmdline_node = of_find_node_by_path("/chosen");
	rc = of_property_read_string(cmdline_node, "bootargs", &hq_cmd_line);
	if (!rc) {
...
{
	hq_get_lcd_name_from_cmdline();
	char *buf_stage = hq_cmd_line;
