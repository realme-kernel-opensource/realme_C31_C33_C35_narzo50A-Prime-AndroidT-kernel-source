...
{
	cts_info("cts_send_command into ");
    if (cts_dev->rtdata.program_mode) {
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
	...
	sprintf(Ctp_name,"%s,ICNL9911C,FW:%04x\n", cts_fw.module_vendor, fwdata->version);
	tp_register_devinfo(fwdata->version);
...
init_hwdata:
    ...
    firmware = cts_request_firmware(cts_dev, CTS_DEV_HWID_ANY, CTS_DEV_FWID_ANY, 0);
...
#define START_BANNER \
    ...
    if (filepath && filepath[0]) {
		...
		cts_info("filp_open is forbiddon with GKI Version!");
#else
...
void cts_firmware_upgrade_work(struct work_struct *work)
{
    ...
    firmware = cts_request_firmware(cts_dev, cts_dev->hwdata->hwid, CTS_DEV_FWID_ANY, cts_dev->fwdata.version);
	//firmware = cts_request_firmware(cts_dev->hwdata->hwid, CTS_DEV_FWID_ANY, cts_dev->fwdata.version, 0);
    if (firmware == NULL) {
