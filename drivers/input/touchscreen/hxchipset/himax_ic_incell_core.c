...
void himax_headset_mode_switch(int headset_sate)
{
  ...
  //  printk("kaoshan %s %d headset_sate = %d \n",__func__,__LINE__,headset_sate);
	do {
		if (headset_sate ) {
...
static void himax_mcu_read_FW_ver(void)
{
	...
	printk("fuye %s %d TOUCH_VER : %X\n",__func__,__LINE__,ic_data->vendor_touch_cfg_ver);
    //get_tp_fw_ver(ic_data->vendor_touch_cfg_ver);
...
int hx_0f_op_file_dirly(char *file_name)
{
    ...
    if (strcmp(MPAP_FWNAME, file_name) == 0) {
		...
		} else if( strcmp(MPAP_FWNAME_FOUR, file_name) == 0 ){
		I("%s, Entering,file name = %s\n", __func__, file_name);
        ret = request_firmware(&tmp_fw_entry, file_name, private_ts->dev);
        if (ret < 0) {
            E("%s,%d: error code = %d, mpap file request fail\n",
       	...
       	if (private_ts->g_fw_sta) {
       		...
       		if(strstr(lcd_himax_name_for_tp, "hx83102d_txd")){
       			request_fw_headfile->size =sizeof(fw_four_i_arr);
       			request_fw_headfile->data = fw_four_i_arr;
		}else{
