...
extern int himax_hx83102_init(void);
extern void  himax_hx83102_exit(void);
extern int himax_hx83112_init(void);
extern void himax_hx83112_exit(void);
extern int Is_this_tp_himax112a;
...
static const uint8_t fw_four_i_arr[]= {
	#include "Nico_Huaqin_CID3502_D00_C01_20220429.i"
	};
...
static int get_boot_mode(void)
{
	...
	const char *cmd_line, *lcd_name_p, *getbootmode;
	int rc;
	...
	if (!rc) {
		...
		getbootmode = strstr(cmd_line, "androidboot.mode=");
		if (lcd_name_p) {
			sscanf(lcd_name_p, "lcd_name=%s", lcd_name_for_tp);
			sscanf(getbootmode, "lcd_name=%s", bootmode);
			I("lcd name: %s,bootmoade:%s\n", lcd_name_for_tp, bootmode);
		}
	} else {
		E("can't not parse bootargs property\n");
		return rc;
	}
   
    if (!strncmp(bootmode, "factorytest", strlen("factorytest")))
        reboot_mode = 1;
...
extern int tp_hx83102d_txd;
extern int tp_hx83112a_inx;
extern int Is_this_tp_hx83102d;

//extern char *lcd_name;
extern char lcd_himax_name_for_tp[50];
extern int get_himax_lcd_name(void);
...
void himax_switch_usb_state(int usb_state)
{
    ...
    if ((private_ts->boot_mode == META_BOOT || private_ts->boot_mode == FACTORY_BOOT))
#else
    if ((private_ts->boot_mode == MSM_BOOT_MODE__FACTORY || private_ts->boot_mode == MSM_BOOT_MODE__RF || private_ts->boot_mode == MSM_BOOT_MODE__WLAN))
#endif
    {
    ...
    if ( private_ts->is_usb_checked != usb_state ) {
        ...
        I("%s: check usb state : %d, suspended: %d\n", __func__, usb_state, private_ts->suspended);
        if (!private_ts->suspended && (private_ts->suspend_state == TP_SPEEDUP_RESUME_COMPLETE)) {
...
void himax_switch_headset_state(int headset_state)
{
    ...
    if ( private_ts->is_headset_checked != headset_state) {
        ...
        I("%s: check headset state : %d, suspended: %d\n", __func__, headset_state, private_ts->suspended);
        if (!private_ts->suspended && (private_ts->suspend_state == TP_SPEEDUP_RESUME_COMPLETE)) {
            mutex_lock(&private_ts->mode_chg_lock);
...
static int i_get_FW(void)
{
	...
	if(!private_ts->recovery_mode){
            I("%s:request fw ,name= %s  tp_hx83112a_inx = %d \n", __func__,BOOT_UPGRADE_FWNAME_INX,tp_hx83112a_inx);
             if(tp_hx83112a_inx)
             {
                 ...
                 if (ret < 0) {
			 	 ...
			 	 else if(tp_hx83102d_txd)
             {
                 I("%s:request fw ,name= %s\n", __func__,BOOT_UPGRADE_FWNAME_FOUR);              
                 ret = request_firmware(&hxfw, BOOT_UPGRADE_FWNAME_FOUR, private_ts->dev);
                 if (ret < 0) {
                	E("%s,%d: request fw fail,and use i file , error code = %d\n", __func__, __LINE__, ret);
               		goto to_i_get_FW;;
                 }
...
void himax_ts_work(struct himax_ts_data *ts)
{
	...
	int ts_path = 0;
	if (ic_boot_done != 1) {
		I("Enter interrupt after firmware loading!\n");
		return;
	}
	...
	if ( g_core_fp.fp_firmware_update_0f != NULL) {
		...
		do {
             ...
             if(Is_this_tp_hx83102d)
             {
                 ...
                 ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_FOUR, ts->dev);
                 if (ret < 0) {
                     I("%s: fw request fail \n", __func__);
                 }else {
                 	break;
		...
		if(fw_entry != NULL) {
			...
			I("%s: fw_name request failed %s %d\n", __func__, BOOT_UPGRADE_FWNAME_SIGNED, ret);
		}
		goto EXIT;
...
int himax_ic_register(void)
{
	int ret = 0;

	if (Is_this_tp_himax112a == 1) {
		I("%s:this is 112a!\n", __func__);
		ret = himax_hx83112_init();
		if(ret) {
			E("%s:Unable to register hx83112a! %d\n", __func__, ret);
			return ret;
		}
	} else if (Is_this_tp_hx83102d == 1) {
		I("%s:this is 102d!\n", __func__);
		ret = himax_hx83102_init();
		if(ret) {
			E("%s:Unable to register hx83102d! %d\n", __func__, ret);
			return ret;
		}
	}
...
//	} 
	...
	if(himax_ic_register()) {
		E("%s: himax_ic_register failed\n", __func__);
		goto err_ic_register_failed;
	}
/*
...
if(strstr(lcd_himax_name_for_tp, "hx83102d_txd")){
			ts->firmware_headfile.data = fw_four_i_arr;
			ts->firmware_headfile.size = sizeof(fw_four_i_arr);
		}else{
...
void himax_edge_mode_set(char *buf,int cnt)
{
	...
	if (buf_tmp[0] == '0') {
		himax_parse_assign_cmd(fw_oppo_tp_direction_0_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oppo_tp_direction/*0x10007f3c*/, DATA_LEN_4, buf_tmp, 0);
		printk("%s: oppo_tp_direction disable .\n", __func__);
	}
	else if(buf_tmp[0] == '1')
	{
		himax_parse_assign_cmd(fw_oppo_tp_direction_1_pwd, buf_tmp, sizeof(buf_tmp));
	...
	else if(buf_tmp[0] == '2')
	{
		...
		printk("%s: oppo_tp_direction wrong parameter .\n", __func__);
		return ;
	}


	g_core_fp.fp_register_read(pfw_op->addr_oppo_tp_direction,DATA_LEN_4, tmp_tmp, 0);
