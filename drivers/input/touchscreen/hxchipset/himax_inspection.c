...
void himax_get_arraydata_edge(int *RAW)
{
	...
	if (ArrayData == NULL) {
		E("%s Allocate ArrayData buf failed\n", __func__);
		return ;
	}
...
int himax_self_test_data_init(void)
{
	...
	char *file_name_4 = "hx_criteria_for_Nico_TXD_V01.csv";
	...
	if (strstr(lcd_himax_name_for_tp, "hx83102d_inx")) {
...
//for GKI
	...
	if (ret == HX_INSP_OK)
		{
		seq_puts(s, "Test passed:\n");
		printk("kaoshan passed %s %d \n",__func__,__LINE__);
		}
		else
		{
		seq_puts(s, "Self_Test Fail:\n");
		printk("kaoshan Fail %s %d \n",__func__,__LINE__);
		}
    /*
