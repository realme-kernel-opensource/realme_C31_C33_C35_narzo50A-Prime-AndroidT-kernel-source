...
char lcd_himax_name_for_tp[50];
EXPORT_SYMBOL(lcd_himax_name_for_tp);
...
int get_himax_lcd_name(void)
{
	...
	if (!rc) {
		...
		if (lcd_name_p) {
			sscanf(lcd_name_p, "lcd_name=%s", lcd_himax_name_for_tp);
    ...
    if (strstr(lcd_himax_name_for_tp, "hx83102d"))
        Is_this_tp_hx83102d = 1;

	E("%s,\n", lcd_himax_name_for_tp);

    return 0;
}
EXPORT_SYMBOL(get_himax_lcd_name);
