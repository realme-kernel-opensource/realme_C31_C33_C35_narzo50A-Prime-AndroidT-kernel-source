...
#include "touch.h"
#include "../../../gpu/drm/sprd/sprd_dsi_panel.h"


extern struct touch_panel tp_interface;
extern int tp_remove_sysfs( struct device *dev);
extern int tp_create_sysfs( struct device *dev);
...
char lcd_fts_name_for_tp[50];

static int get_ft8006s_mode(void)
{
	struct device_node *cmdline_node;
	const char *cmd_line, *lcd_name_p;
	int rc;
	FTS_ERROR("%s:get_nvt_mode!!!!\n", __func__);
	cmdline_node = of_find_node_by_path("/chosen");
	rc = of_property_read_string(cmdline_node, "bootargs", &cmd_line);
	if (!rc) {
		lcd_name_p = strstr(cmd_line, "lcd_name=");
		if (lcd_name_p) {
			sscanf(lcd_name_p, "lcd_name=%s", lcd_fts_name_for_tp);
			FTS_ERROR("%d:lcd name: %s\n", __LINE__, lcd_fts_name_for_tp);
		}
	}

    if (strstr(lcd_fts_name_for_tp, "lcd_ft8006s"))
   		 fts_ic_type = 1;
    if(strstr(lcd_fts_name_for_tp, "lcd_ft8006saa")){
	    fts_ic_type = 1;
	    ft8006saa = 1;
    }
    if (strstr(lcd_fts_name_for_tp, "lcd_ft8722"))
    	fts_ic_type = 2;

	 if (strstr(lcd_fts_name_for_tp, "ft8615_inx"))
    	fts_ic_type = 3;
...
#endif
    //printk("[FTS_TS]%s: Enter ts_data->gesture_mode = %d,tp_gesture = %d\n", __func__,ts_data->gesture_mode,tp_gesture);
    if (ts_data->gesture_mode && tp_gesture) {
                                ...
                                unsigned long action, void *data)
{
        ...
        case FTS_TP_RESET_0:
            fts_tp_reset_pin(0);
            FTS_INFO("FTS_TP_RESET_0\n");
            break;
        case FTS_TP_RESET_1:        
            fts_tp_reset_pin(1);
            FTS_INFO("FTS_TP_RESET_1\n");
            break;
        default:
            return NOTIFY_DONE;
        }
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	if( ic_type == IC_TYPE_FT8006S_AA)
		get_hardware_info_data(HWID_CTP_DRIVER,"txd_v5000a_ft8006s");
...
static int __init fts_ts_init(void)
{
	...
	ret = get_ft8006s_mode();
	if(ret) {
		FTS_ERROR("get_ft8006s_mode failed  !\n");
	}
